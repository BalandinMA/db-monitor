...
export class TransactionInfoComponent implements OnInit {
  ...
  transactionDetails: object
    = {
    "INBOUND.INBD_STATUS.22": "status was changed!"
  }
  ;
  ...
  getTransactionInfo(): void {
    this.transactionInfoService.getTransactionInfo(this.requestModel)
      .subscribe(data => {
        this.translationDetails = data.translationDetails;
      })
  };
  ...
  getValue(table: string, column: string, cell: string): string {
    if (this.transactionDetails[table + '.' + column + '.' + cell]) {
      return this.transactionDetails[table + '.' + column + '.' + cell];
    }
    else return cell;
  }
}
